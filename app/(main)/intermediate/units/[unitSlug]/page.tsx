...
export default async function IntermediateUnitPage({ params }: { params: Promise<{ unitSlug: string }> }) {
  ...
  if (!unit || unit.level.level !== "INTERMEDIATE") notFound();
            ...
            <Card key={lesson.id} className={`transition-shadow ${!prevDone ? "opacity-60" : "hover:shadow-md"}`}>
              <CardContent className="pt-3 pb-3">
                <div className="flex items-center gap-3">
                  ...
                  {prevDone && (
                    <Link href={`/intermediate/units/${unit.slug}/lessons/${lesson.slug}`}>
        ...
        <Card className="border-orange-200 bg-orange-50">
          <CardContent className="pt-4">
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <Link href={`/intermediate/units/${unit.slug}/quiz`}>
