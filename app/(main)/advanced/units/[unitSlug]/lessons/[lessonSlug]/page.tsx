...
export default function AdvancedLessonPage() {
  ...
  async function markComplete() {
    ...
    setMarking(false);
    if (lesson.nextLessonSlug) { router.push(`/advanced/units/${unitSlug}/lessons/${lesson.nextLessonSlug}`); }
    else { router.push(`/advanced/units/${unitSlug}`); }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <Breadcrumb items={[{ label: "Derejeler", href: "/levels" }, { label: "Ýokary", href: "/advanced" }, { label: lesson.unitTitle, href: `/advanced/units/${unitSlug}` }, { label: lesson.titleTk }]} />
