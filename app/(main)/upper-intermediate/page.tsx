...
export const metadata = { title: "Ýokary Orta | Iňlis Dili" };

export default async function UpperIntermediatePage() {
  const session = await auth();
  const userId = session!.user.id;

  const level = await prisma.courseLevel.findUnique({
    where: { level: LevelSlug.UPPER_INTERMEDIATE },
    include: {
      units: {
        include: {
            ...
            <Card key={unit.id} className={`transition-shadow ${locked ? "opacity-60" : "hover:shadow-md"}`}>
              <CardContent className="pt-4">
                <div className="flex items-start gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 ${allDone ? "bg-green-100 text-green-700" : unitDone > 0 ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-500"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {!locked && (
                      <Link href={`/upper-intermediate/units/${unit.slug}`} className="inline-block mt-3">
        ...
        <Card className="border-blue-200 bg-blue-50">
          <CardContent className="pt-4">
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <Link href="/upper-intermediate/test"><Button variant="default">Synagy başla</Button></Link>
            </div>
          </CardContent>
        </Card>
      )}
