...
export default async function UpperIntQuizPage({ params }: { params: Promise<{ unitSlug: string }> }) {
  ...
  return (
    <div className="space-y-4">
      <Link href={`/upper-intermediate/units/${unitSlug}`} className="text-sm text-gray-500 hover:text-gray-700">← {unit.titleTk}</Link>
      <QuizContainer questions={questions} quizId={unit.quiz.id} title={unit.quiz.titleTk} backHref={`/upper-intermediate/units/${unitSlug}`} />
    </div>
  );
}
