...
export default async function PreIntermediateUnitPage({ params }: { params: Promise<{ unitSlug: string }> }) {
  ...
  if (!unit || unit.level.level !== "PRE_INTERMEDIATE") notFound();
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <Breadcrumb items={[
          ...
          { label: "Orta Öňi", href: "/pre-intermediate" },
          { label: unit.titleTk },
        ]} />
            ...
            <Card key={lesson.id} className={`transition-shadow ${!prevDone ? "opacity-60" : "hover:shadow-md"}`}>
              <CardContent className="pt-3 pb-3">
                <div className="flex items-center gap-3">
                  ...
                  {prevDone && (
                    <Link href={`/pre-intermediate/units/${unit.slug}/lessons/${lesson.slug}`}>
          ...
          <CardContent className="pt-4">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div>
                ...
                {bestAttempt ? (
                  <p className="text-sm text-gray-600 mt-0.5">Iň gowy netije: {bestAttempt.score}/{bestAttempt.total}</p>
                ) : (
                  <p className="text-sm text-gray-600 mt-0.5">10 sorag</p>
                )}
              </div>
              <Link href={`/pre-intermediate/units/${unit.slug}/quiz`}>
