...
export const metadata = { title: "Başlangyç+ | Iňlis Dili" };
...
export default async function ElementaryPage() {
  ...
  const level = await prisma.courseLevel.findUnique({
    where: { level: LevelSlug.ELEMENTARY },
    include: {
      units: {
        include: {
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="text-right">
          <div className="text-sm text-gray-500">
            {completedLessons}/{totalLessons} sapak
          </div>
          <Progress value={(completedLessons / totalLessons) * 100} className="w-40 mt-1" />
        </div>
      </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 flex items-center gap-3">
                      <Progress value={(unitDone / unitTotal) * 100} className="h-1 flex-1" />
                      <span className="text-xs text-gray-400 whitespace-nowrap">{unitDone}/{unitTotal}</span>
                    </div>
                    {!locked && (
                      <Link href={`/elementary/units/${unit.slug}`} className="inline-block mt-3">
